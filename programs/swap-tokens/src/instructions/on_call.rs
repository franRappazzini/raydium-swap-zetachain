...
pub struct OnCall<'info> {
    ...
    #[account(
        mut,
        seeds = [SEED_VAULT, output_vault_mint.key().as_ref()],
        bump,
    )]
...
pub fn process_on_call<'a, 'b, 'c: 'info, 'info>(
    ...
    utils::swap_tokens(
        ...
        &acc.amm_config,
        &acc.pool_state,
        &acc.wsol_vault, // wsol program ata
        &acc.usdc_vault, // usdc user ata
        &acc.input_vault,
        &acc.output_vault,
    ...
    // alert to zetachain
    let before_output_token_balance = acc.usdc_vault.amount;
    msg!(
